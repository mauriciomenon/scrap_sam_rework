...
class SSADashboard:
    ...
    def _get_initial_stats(self):
        ...
        try:
            ...
            # SSAs críticas (S3.7)
            ssas_criticas = len(self.df[
                self.df.iloc[:, SSAColumns.GRAU_PRIORIDADE_EMISSAO].str.upper() == 'S3.7'
            ])
            ...
            return {
                ...
                'por_estado': estados,
                'periodo': {
                    'inicio': data_mais_antiga,
                    'fim': data_mais_recente
                },
                'responsaveis': {
                    'programacao': self.df.iloc[:, SSAColumns.RESPONSAVEL_PROGRAMACAO].nunique(),
                    'execucao': self.df.iloc[:, SSAColumns.RESPONSAVEL_EXECUCAO].nunique()
                }
            }
        except Exception as e:
